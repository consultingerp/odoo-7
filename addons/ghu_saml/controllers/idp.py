...
class GhuIdp(http.Controller):
    ...
    @http.route('/saml/metadata', auth='public')
    def metadata(self, request, **kw):
        environ = request.httprequest.environ
        return metadata(environ)
...
def metadata(environ):
    try:
        ...
        logger.error("An error occured while creating metadata: %s", ex.msg)
        return false
