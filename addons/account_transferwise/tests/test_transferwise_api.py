...
class TestTransferwiseApi(AccountingTestCase):

    def setUp(self):
        super(TestTransferwiseApi, self).setUp()
        self.db_name = self.env.cr.dbname
        self.db_uid = self.env['ir.config_parameter'].get_param('database.uuid')
        self.url = 'https://onlinesync.odoo.com/transferwise/api/2'
    ...
    def transferwise_post(self, *args, **kwargs):
        ...
        if args[0] == self.url + '/item/public_token/exchange':
            ...
            resp = {
              "accounts": [{
                 ...
                 "mask": "0000",
                 "name": "Transferwise Checking",
                 "official_name": "Transferwise Gold Checking",
                 "subtype": "checking",
                 "type": "depository"
              }],
    ...
    def transferwise_post_error(self, *args, **kwargs):
        if (args[0].endswith('error_transferwise')):
            error = {
        ...
        elif (args[0].endswith('error_transferwise2')):
            error = { 
                "error": {
    ...
    def test_transferwise_create_institution(self):
        """ Test adding a new institution with transferwise """
        # Simulate a user that just complete the authentication process with transferwise, wa have received a token and a list
        # Of account_ids and need to exchange the token to get the id identifying the user in transferwise database and then create
        # the account.online.provider with the linked accounts in Odoo
        
        # Patch request.post with defined method
        patcher = patch('odoo.addons.account_transferwise.models.transferwise.requests.post', side_effect=self.transferwise_post)
        patcher.start()
        metadata = {
            'link_session_id': '378547f9-7a1a-4edd-a566-3a9bf99284c8', 
            'account': {'name': 'Transferwise Checking', 'id': 'QKKzevvp33HxPWpoqn6rI13BxW4awNSjnw4xv', 'type': 'depository'}, 
            'public_token': 'public-sandbox-019e1018-be47-4842-aada-1dea2ee0ab22', 
            'accounts': [{'name': 'Transferwise Checking', 'id': 'QKKzevvp33HxPWpoqn6rI13BxW4awNSjnw4xv', 'type': 'depository'}], 
        ...
        self.assertEqual(acc_online_provider.provider_type, 'transferwise')
        ...
        self.assertEqual(acc_online_provider.account_online_journal_ids.name, 'Transferwise Checking')
    ...
    def test_transferwise_fetch_transactions(self):
        """ Test receiving some transactions with transferwise """
    ...
    def test_transferwise_fetch_errors(self):
        """ Test some transferwise error that might happen """
        # Patch request.post with defined method
        patcher = patch('odoo.addons.account_transferwise.models.transferwise.requests.post', side_effect=self.transferwise_post_error)
        patcher.start()

        with self.assertRaises(UserError) as e:
            self.env['account.online.provider'].transferwise_fetch('/error_transferwise', {})
        self.assertEqual(e.exception.name, 'There was en error with Transferwise Services!\n{message: Explained error text,\nerror code: INTERNAL_SERVER_ERROR,\nerror type: API_ERROR,\nrequest id: 12345}')

        with self.assertRaises(UserError) as e:
            self.env['account.online.provider'].transferwise_fetch('/error_transferwise2', {})
        self.assertEqual(e.exception.name, 'There was en error with Transferwise Services!\n{message: Site is down,\nerror code: INSTITUTION_DOWN,\nerror type: INSTITUTION_ERROR,\nrequest id: 12345}')

        with self.assertRaises(UserError) as e:
            self.env['account.online.provider'].transferwise_fetch('/no_contract', {})
        self.assertEqual(e.exception.name, 'No valid Odoo Enterprise contract found for this database')

        with self.assertRaises(UserError) as e:
            self.env['account.online.provider'].transferwise_fetch('/pokemon', {})
        ...
        with self.assertRaises(UserError) as e:
            acc_online_provider.transferwise_fetch('/error_transferwise', {})
        ...
        self.assertEqual(acc_online_provider.status, 'SUCCESS', 'state of account online provider should be in SUCCESS')
        with self.assertRaises(UserError) as e:
            acc_online_provider.transferwise_fetch('/error_transferwise2', {})
        self.assertEqual(acc_online_provider.status, 'FAILED', 'state of account_online_provider should have change to FAILED')
        ...
        self.assertEqual(acc_online_provider.status, 'SUCCESS', 'state of account online provider should be in SUCCESS')
        with self.assertRaises(UserError) as e:
            acc_online_provider.transferwise_fetch('/no_contract', {})
        self.assertEqual(acc_online_provider.status, 'FAILED', 'state of account_online_provider should have change to FAILED')
        ...
        self.assertEqual(acc_online_provider.status, 'SUCCESS', 'state of account online provider should be in SUCCESS')
        with self.assertRaises(UserError) as e:
            acc_online_provider.transferwise_fetch('/pokemon', {})
    ...
    def test_assign_partner_automatically(self):
        """ Test receiving some transactions with transferwise and assigning partner"""
        # Create fake account.online.provider
        bank_journal = self.env['account.journal'].search([('type', '=', 'bank')], limit=1) or False
        acc_online_provider = self.create_account_provider()
        if bank_journal:
