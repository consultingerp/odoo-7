...
class TransferwiseProviderAccount(models.Model):
    ...
    transferwise_error_type = fields.Char(readonly=True, help='Additional information on error')
    ...
    def check_transferwise_error(self, resp):
        try:
            ...
            if type(resp_json) == dict and resp_json.get('error_code') and resp.status_code >= 400:
                message = _('There was en error with Transferwise Services!\n{message: %s,\nerror code: %s,\nerror type: %s,\nrequest id: %s}')
    ...
    @api.multi
    def transferwise_fetch(self, url, data):
        credentials = self._get_transferwise_credentials()
        ...
        try:
            ...
            # This is only intended to work with Odoo proxy, if user wants to use his own transferwise account
        ...
        self.check_transferwise_error(resp)
        resp_json = resp.json()
        if self and self.id:
    ...
    def get_login_form(self, site_id, provider, beta=False):
        ...
        view_id = self.env.ref('ghu.transferwise_settings_view_form').id
    ...
    #     resp_json = self.transferwise_fetch('/item/public_token/exchange', data)
    #     item_vals = {
    ...
    #     # Call transferwise to get balance on all selected accounts.
    #     data = { 'access_token': resp_json.get('access_token'), 'options': {'account_ids': accounts_ids}}
    #     resp_json = self.transferwise_fetch('/accounts/balance/get',data)
    #     account_vals = []
    ...
    def _update_status(self, status, resp_json=None):
        ...
        with self.pool.cursor() as cr:
            self = self.with_env(self.env(cr=cr, user=SUPERUSER_ID)).write({
                ...
                'message': message,
                'transferwise_error_type': error_type,
    ...
    def update_credentials(self):
        if self.provider_type != 'transferwise':
            return super(TransferwiseProviderAccount, self).update_credentials()
        # Create public token and open link in update mode with that token
        resp_json = self.transferwise_fetch('/item/public_token/create', {})
        ret_action = self.get_login_form(self.provider_identifier, 'transferwise')
    ...
    def unlink(self):
        for provider in self:
            if provider.provider_type == 'transferwise':
                # call transferwise to ask to remove item
                try:
                    ctx = self._context.copy()
                    ctx['no_post_message'] = True
                    provider.with_context(ctx).transferwise_fetch('/item/remove', {})
                except UserError:
                    # If call to fails, don't prevent user to delete record 
                    pass
        super(TransferwiseProviderAccount, self).unlink()
...
class TransferwiseAccount(models.Model):
    ...
    def retrieve_transactions(self):
        ...
        while True:
            ...
            resp_json = self.account_online_provider_id.transferwise_fetch('/transactions/get', params)
            # Update the balance
            ...
            for transaction in resp_json.get('transactions'):
                if transaction.get('pending') == False:
                    trans = {
                        ...
                        'amount': -1 * transaction.get('amount'), #https://transferwise.com/docs/api/#transactions amount positive if purchase
